    ...
    >
      ...
      >
        ...
        <ProjectSection variants={itemVariants}>
          ...
          <SubSection>
            <h4>환경 선택</h4>
            <p>클라우드 서비스 대신, 개인서버를 구축하는 방식을 선택하였습니다.</p>
            <p><strong>선택 이유:</strong> 가상화 기술(`Proxmox VE`,etc)에 대한 실질적인 이해를 높이고,
             보유한 하드웨어 자원을 효율적으로 분할하여
             물리 서버 하드웨어에 `Proxmox VE`를 설치하여 서버환경을 유연하게 구성하기 위함이었습니다.</p>
          </SubSection>
          ...
          <SubSection>
            ...
            <p>`Proxmox VE`를 `Ryzen 5 5600g`, `32GB RAM`, `500GB Nvme`  + `4TB sdd` 구성의 서버에 설치하였었으나.
               `ssd`에 문제가 발생하여 현재는 `500GB Nvme` + `1TB sdd` 구성의 서버로 임시로 변경하였습니다.
               설치 후 웹 인터페이스를 통해 네트워크 설정, 저장소(`Storage`) 설정 등 기본적인 초기 구성을 완료하였습니다.</p>
          </SubSection>
          
          <SubSection>
            <h4>개발 서버 VM 생성 및 Debian 설치</h4>
            <p>`Proxmox VE` 내에 개발 서버 용도의 가상 머신(`VM`)을 생성하고, 운영체제로 `Debian Linux`를 설치하였습니다.</p>
            <p><strong>Debian 선택 이유:</strong> 서버 운영체제로서의 높은 안정성, 폭넓은 패키지 지원, 그리고 `CLI` 환경에 대한 숙련도를 높이기에 적합하다고 판단하였습니다.</p>
            <p>`VM`에는 `6 CPU 코어`, `16GB 메모리`, `256GB`의 디스크 공간을 할당하였습니다. `Debian` 설치 후 `SSH` 서버를 설정하여 원격 접속 환경을 마련하고, 고정 `IP` 주소를 할당하였으며, `apt` 패키지 매니저를 통해 시스템을 최신 상태로 업데이트하였습니다. 모든 서버 관리는 `CLI` 환경을 중심으로 진행하였습니다.</p>
          </SubSection>
        </ProjectSection>
        ...
        <ProjectSection variants={itemVariants}>
          ...
          <SubSection>
            ...
            <ul>
              <li>`apt` 패키지 매니저를 사용하여 `Node.js`와 `OpenJDK` (`Java Development Kit`)를 설치하였습니다.</li>
              <li>`Node.js` 프로젝트 관리를 위해 `npm`을, `Java` 프로젝트 빌드를 위해 `Gradle`을 설치하고 설정하였습니다.</li>
              <li>로컬 개발 환경에서 사용하는 `IntelliJ IDEA`와 `Visual Studio Code`에서 `SSH` 원격 접속 기능을 활용하여 서버 자원을 직접 사용하며 개발 작업을 수행할 수 있도록 구성하였습니다.</li>
            </ul>
          </SubSection>
          
          <ImageContainer>
            <ProjectImage src="/images/vscode-remote-ssh.png" alt="VS Code 원격 SSH 연결" />
            <ImageCaption>그림 2: `Visual Studio Code`를 사용한 `SSH` 원격 개발 환경</ImageCaption>
          </ImageContainer>
          
          <SubSection>
            <h4>2.2. 웹 서버 및 리버스 프록시 (`Caddy`)</h4>
            <p>웹 서버로는 `Caddy`를 선택하였습니다.</p>
            <p><strong>Caddy 선택 이유:</strong> 설정 파일(`Caddyfile`)의 간결성, 자동 `HTTPS` 설정(`Let's Encrypt` 연동) 기능이 뛰어나 개인 서버 환경에 적합하다고 판단하였습니다.</p>
            <p>`Caddy`를 설치하고, 개발 중인 애플리케이션(`Node.js` 또는 `Java` 기반)으로 요청을 전달하는 리버스 프록시 설정과 정적 파일 호스팅 설정을 구성하였습니다.</p>
...
capu.it.com {                  
    ...
    encode gzip zstd 
          ...
          <SubSection>
            ...
            <ul>
              <li>관계형 데이터베이스로는 `MariaDB`를 설치하였습니다.</li>
              <li>`mysql_secure_installation` 스크립트를 실행하여 루트 비밀번호 설정, 익명 사용자 제거, 원격 루트 로그인 비활성화 등 기본적인 보안 설정을 완료하였습니다.</li>
              <li>애플리케이션에서 사용할 데이터베이스와 사용자 계정을 생성하고, 해당 계정에 필요한 권한을 부여하였습니다.</li>
            </ul>
...
FLUSH PRIVILEGES;`}
        ...
        <ProjectSection variants={itemVariants}>
          ...
          <SubSection>
            <h4>방화벽 설정 (`UFW`)</h4>
            <p>`UFW` (`Uncomplicated Firewall`)를 사용하여 서버 접근 포트를 제어하였습니다.</p>
            <p>기본 정책을 모든 인바운드 연결 거부(`deny incoming`)로 설정하고, `SSH`(포트 변경 2945), `HTTP`(80), `HTTPS`(443) 등 필수적인 서비스 포트만 명시적으로 허용하는 규칙을 추가하였습니다
              <br/>
              <strong>참고:</strong> `SSH`등 연결은 로컬망 내부에서만(`Local Only`) 접속망 허용하고 `vpn`을 구축하여 보안을 강화하였습니다.
              </p>
...
sudo ufw enable`}
          ...
          <SubSection>
            ...
            <p>`Fail2ban`을 설치하고 설정하여 `SSH` 서비스에 대한 무차별 대입 공격(`Brute-force attack`) 시도를 모니터링하고, 일정 횟수 이상 실패 시 해당 `IP`를 자동으로 차단하도록 구성하였습니다.</p>
...
findtime = 600`}
          ...
          <SubSection>
            <h4>기타 보안 설정</h4>
            <ul>
              <li>`SSH` 비밀번호 인증을 비활성화하고 키 기반 인증만 허용하도록 설정하여 보안을 강화하였습니다.</li>
              <li>`root` 계정의 직접 로그인을 제한하고 일반 사용자로 로그인 후 `sudo`를 사용하도록 하였습니다.</li>
              <li>`apt`를 통해 시스템 및 설치된 패키지를 정기적으로 업데이트하여 보안 취약점에 대응하였습니다.</li>
            </ul>
        ...
        <ProjectSection variants={itemVariants}>
          ...
          <SubSection>
            ...
            <ul>
              ...
              <li>`Node.js` 애플리케이션은 `PM2`를 사용하여 프로세스를 관리하고 백그라운드 실행 및 자동 재시작을 설정하였습니다.</li>
              <li>`Java` 애플리케이션 (예: <strong>`Spring Boot` 기반 `JAR`</strong>)은 빌드된 `JAR` 파일을 `systemd` 서비스로 등록하여 서버 부팅 시 자동 실행되고 관리되도록 구성하였습니다.</li>
            </ul>
...
WantedBy=multi-user.target`}
          ...
          <SubSection>
            <h4>운영</h4>
            <p>서버 리소스 사용량(`CPU`, `메모리`)은 `htop` 등의 도구로 모니터링하였으며, 애플리케이션 및 시스템 로그를 주기적으로 확인하여 문제 발생 여부를 파악하였습니다.</p>
          </SubSection>
          
          <ImageContainer>
            <ProjectImage src="/images/htop-monitoring.png" alt="htop을 이용한 서버 모니터링" />
            <ImageCaption>그림 4: `htop`을 이용한 서버 리소스 모니터링</ImageCaption>
          </ImageContainer>
        </ProjectSection>
        ...
        <ProjectSection variants={itemVariants}>
          ...
          <SubSection>
            ...
            <p><strong>상황:</strong> 프로젝트 진행 중, 서버의 메인 저장 장치로 사용하던 `SSD`에 예상치 못한 물리적 장애가 발생하였습니다. 이로 인해 운영체제, 설정 파일, 데이터베이스 등 서버 내 모든 데이터와 설정이 유실되었습니다.</p>
            <p><strong>해결 과정:</strong> 당시 정기적인 백업 체계가 구축되어 있지 않아, 서버 환경 전체를 처음부터 다시 구축해야 하였습니다. 이 과정에서 이전에 수동으로 진행했던 설정들을 자동화 스크립트 등으로 일부 개선하여 재구축 시간을 단축하려 노력하였습니다.</p>
            <p><strong>배운 점:</strong> 이 경험은 데이터 및 시스템 설정의 정기적인 백업과 복구 전략 수립이 서버 운영에 있어 단순한 권장 사항이 아닌 가장 기본적인 필수 요소임을 뼈저리게 느끼게 하였습니다. 이후 중요한 설정 파일과 데이터를 주기적으로 외부 저장소에 백업하는 스크립트를 작성하고 실행하는 체계를 마련하였습니다.</p>
          </SubSection>
          ...
          <SubSection>
            ...
            <p><strong>해결 과정:</strong> `Gradle`의 의존성 트리를 확인하여 충돌이 발생하는 라이브러리와 버전을 특정하고, `configurations.all*.resolutionStrategy` 설정을 통해 특정 라이브러리의 버전을 강제하거나 충돌을 해결하는 방법을 적용했습니다.</p>
